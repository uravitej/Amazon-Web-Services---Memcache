...
for key in range(1,2000):
    ...
    if num == 1:
            ...
            sQuery = "SELECT datetime, gap, FLOOR(RAND()*" + str(num) + ") from monthlyearthquake WHERE gap > FLOOR(RAND()*" + str(num) + ") limit 1"
            ...
            sQuery = "SELECT latitude, longitude, FLOOR(RAND()*" + str(num) + ") from monthlyearthquake WHERE nst > FLOOR(RAND()*" + str(num) + ") limit 1"
            cursor.execute(sQuery);
            #cursor.execute ("SELECT datetime, gap, FLOOR(RAND()*20) from monthlyearthquake WHERE gap > FLOOR(RAND()*20) limit" + num +  ")"
            rows = cursor.fetchall()
            #for row in rows:
            ...
            sQuery = "SELECT location, mag, FLOOR(RAND()*" + str(num) + ") from monthlyearthquake WHERE depth > FLOOR(RAND()*" + str(num) + ") limit 1"
            cursor.execute(sQuery);
            #cursor.execute ("SELECT datetime, gap, FLOOR(RAND()*20) from monthlyearthquake WHERE gap > FLOOR(RAND()*20) limit" + num +  ")"
            rows = cursor.fetchall()
            #for row in rows:
...
print 'Total elapsed time to run 2000 random queries is',endtime-begintime
cursor.close()
conn.close()
